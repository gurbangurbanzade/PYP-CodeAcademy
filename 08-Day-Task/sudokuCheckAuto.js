...
// -----------------------avtomatik dolum-----------------
let sudokuBtnAuto = document.querySelector(".sudokuBtnAuto");
console.log(valueBox[0].innerText);
sudokuBtnAuto.addEventListener(
  "click",
  (sudoku = function (e) {
    // e.preventDefault();
    for (let i = 0; i < valueBox.length; i++) {
      valueBox[i].innerText = "";
      if (valueBox[i].innerText == "") {
        let y = Math.floor(Math.random() * 10);
        if (y) valueBox[i].innerText = y;
        else valueBox[i].innerText = 1;
        console.log(valueBox[i].innerText);
      }
    }
    ///////////// yeniden yoxlayir----------------------------
    for (let i = 0; i < checkItems.length; i += 9) {
